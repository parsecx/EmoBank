...
public partial class frmMovimentoPrelievi : System.Web.UI.Page
{
    ...
    private void BindMovimentoPrelieviByCodId(string CodId)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            BindAnimals(ddlDenominazione.SelectedValue, ddlSpecieAnimale.SelectedValue, ProgressiveAnimal, ProtocolNumberValue);
            try
            {
    ...
    protected void btnRegister_Click(object sender, EventArgs e)
    {
        ...
        if (Validation())
        {
            ...
            SqlParameter[] param =
            {
                new SqlParameter("@AnimalName",txtAnimalName.Text)
            };

            int ProgressiveValue;

            DataTable dt = cls.ExecuteDataTable("Donor_Registry_DetailByAnimalName", param, CommandType.StoredProcedure);

            ProgressiveValue = Convert.ToInt32(dt.Rows[0]["Progressive"].ToString());

            SqlParameter[] Opara =
            {
             new SqlParameter("@Rval",ParameterDirection.ReturnValue),
             new SqlParameter("@Name",""),
             new SqlParameter("@Address",txtDIndirizzo.Text),
            ...
            // new SqlParameter("@ProtocolNumber",txtProtocollo.Text),
             ...
             new SqlParameter("@TypePrepared", ddlTipoPreparato.SelectedItem.Text),
    ...
    protected void ddlDenominazione_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            GetDonatorsById(ddlDenominazione.SelectedValue);
            BindAnimals(ddlDenominazione.SelectedValue, ddlSpecieAnimale.SelectedValue, 0, 0);
        }
    }
    ...
    #region
    public void BindAnimals(string DCodID, string AnimalSpecies, int ProgressiveAnimal, int ProtocolNumberValue)
    {
        SqlParameter[] oPera =
        {
            new SqlParameter("@DCodId",DCodID),
            new SqlParameter("@AnimalSpecies",AnimalSpecies),
            new SqlParameter("@Progressive",ProgressiveAnimal),
            new SqlParameter("@ProtocolNumber",ProtocolNumberValue)
        };
    ...
    protected void grdAnimals_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            {
                string kindOfAnimal = e.Row.Cells[1].Text;
                if (kindOfAnimal == "Cane")
                {
                    ddlAnimaleSpecie.Items.Insert(0, new ListItem(e.Row.Cells[1].Text, "Dog"));
                }
                else if (kindOfAnimal == "Gatto")
                {
                    ddlAnimaleSpecie.Items.Insert(0, new ListItem(e.Row.Cells[1].Text, "Cat"));
                }
                else
                {
                    ddlAnimaleSpecie.Items.Insert(0, new ListItem(e.Row.Cells[1].Text, "Horse"));
                }
                ddlAnimaleSpecie.SelectedIndex = 0;
                txtAnimalName.Text = e.Row.Cells[2].Text;
                txtAge.Text = e.Row.Cells[4].Text;
                txtPeso.Text = e.Row.Cells[3].Text;
            }
            //e.Row.Attributes["onclick"] = Page.ClientScript.GetPostBackClientHyperlink(grdData, "Select$" + e.Row.RowIndex);
    ...
    protected void grdAnimals_MODIFICA(object sender, EventArgs e)
    {
        BindAnimals(ddlDenominazione.SelectedValue, ddlSpecieAnimale.SelectedValue, 0, 0);
    }
    ...
    protected void gvAttachments_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "DEL")
        {
            string ltrID = gvAttachments.DataKeys[Convert.ToInt32(e.CommandArgument.ToString())].Value.ToString() ?? "0";
